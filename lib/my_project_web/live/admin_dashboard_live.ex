  ...
  def render(assigns) do
    ...
    <aside class="fixed top-0 left-0 h-screen w-64 bg-gray-800 text-white z-50">
      ...
      <nav class="h-full overflow-y-auto p-4">
        <ul class="space-y-2">
          <li>
            <.link patch={~p"/admin?menu=menu1"} class={"block p-2 rounded #{if @selected_menu == "menu1", do: "bg-gray-900 font-bold", else: "hover:bg-gray-700"}"}>
              Menu 1
            </.link>
          </li>
          <li>
            <.link patch={~p"/admin?menu=menu2"} class={"block p-2 rounded #{if @selected_menu == "menu2", do: "bg-gray-900 font-bold", else: "hover:bg-gray-700"}"}>
              Menu 2
            </.link>
          </li>
          <li>
            <.link patch={~p"/admin?menu=menu3"} class={"block p-2 rounded #{if @selected_menu == "menu3", do: "bg-gray-900 font-bold", else: "hover:bg-gray-700"}"}>
              Menu 3
            </.link>
          </li>
           <li>
            <.link patch={~p"/admin?menu=home"} class={"block p-2 rounded #{if @selected_menu == "home", do: "bg-gray-900 font-bold", else: "hover:bg-gray-700"}"}>
              Home
            </.link>
          </li>
        </ul>
